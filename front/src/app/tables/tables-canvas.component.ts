...
@Component({
      ...
      <div class="canvas-container">
        ...
        <div class="floor-tabs">
          ...
          @if (floors().length > 0 && selectedFloorId()) {
            <button class="floor-tab floor-action" (click)="editFloor()" title="Rename floor">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          >
            @if (floors().length === 0) {
              ...
              >
                ...
                <defs>
                  <!-- Chair gradient for 3D effect -->
                  <linearGradient id="chairGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" style="stop-color:#6b7db3"/>
                    <stop offset="50%" style="stop-color:#5a6a9e"/>
                    <stop offset="100%" style="stop-color:#4a5a8e"/>
                  </linearGradient>
                  <linearGradient id="chairHighlight" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" style="stop-color:#8b9dc3"/>
                    <stop offset="100%" style="stop-color:#5a6a9e"/>
                  </linearGradient>
                  <!-- Table shadows -->
                ...
                @for (table of tablesOnCurrentFloor(); track table.id) {
                  ...
                  >
                    ...
                    @for (seat of getSeatPositions(table); track $index) {
                      <g [attr.transform]="'translate(' + seat.x + ',' + seat.y + ') rotate(' + seat.angle + ')'">
                        <!-- Chair body (3D rounded rectangle) -->
                        <ellipse
                          cx="0" cy="0"
                          rx="12" ry="16"
                    ...
                    <g filter="url(#tableShadow)">
                      @if (table.shape === 'circle') {
                        <ellipse
                          ...
                          [attr.fill]="table.status === 'occupied' ? '#22c55e' : '#e8dcc8'"
                          [attr.stroke]="selectedTable()?.id === table.id ? '#22c55e' : (table.status === 'occupied' ? '#16a34a' : '#d4c4a8')"
                          stroke-width="3"
                        />
                    ...
                    <text
                      ...
                      [attr.fill]="table.status === 'occupied' ? 'white' : '#5a4a3a'"
                    >
  ...
  styles: [`
    ...
    .floor-tab.floor-action:first-of-type { margin-left: auto; }
...
export class TablesCanvasComponent implements OnInit, OnDestroy {
  ...
  tableShapes: TableShape[] = [
    { id: 'square4', name: 'Square', shape: 'rectangle', width: 80, height: 80, seats: 4 },
    { id: 'rect4', name: 'Rectangle', shape: 'rectangle', width: 100, height: 70, seats: 4 },
    { id: 'rect6', name: 'Long', shape: 'rectangle', width: 140, height: 70, seats: 6 },
    { id: 'circle4', name: 'Round', shape: 'circle', width: 80, height: 80, seats: 4 },
    { id: 'circle6', name: 'Large Round', shape: 'circle', width: 100, height: 100, seats: 6 },
    { id: 'oval6', name: 'Oval', shape: 'oval', width: 120, height: 70, seats: 6 }
  ];
  ...
  getSeatPositions(table: CanvasTable): { x: number; y: number; angle: number }[] {
    ...
    const chairOffset = 20; // Distance from table edge
    ...
    if (table.shape === 'circle' || table.shape === 'oval') {
      ...
      // Rectangle - distribute evenly on all sides
