...
4. **Results:**
   ...
   - **Waiter — assigned name visible, read-only:** **PASS** — Login `ralf.roeber@amvara.de` / `123456` (tenant 1), `/tables` → Table view: row text includes **Werni** for assigned tables; **no** empty broken `<select>` in `.tables-data-table`. **API check:** same session cookie → `GET /api/tables/with-status` returns `assigned_waiter_name` / `effective_waiter_name` where applicable (e.g. T07).
