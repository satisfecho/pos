...
## Test report (2026-04-28 UTC — tester)

1. **Date/time (UTC) and log window**  
   - **Window:** 2026-04-28T00:38Z–00:39Z.  
   - **Commands:** `git`, `gh`, `curl` from `/Users/raro42/projects/pos2` after `./scripts/git-sync-development.sh` (not `docker logs` — not required by Testing instructions).

2. **Environment**  
   - **Compose / local app:** N/A.  
   - **Branch:** `development` synced with `origin/development`.  
   - **Remotes (after `git fetch origin`):** `origin/master` = `7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`, `origin/development` = `120daf35f63a7808156aa339c1abdc59943c86bb`.  
   - **Evidence:** `gh` CLI to `satisfecho/pos` Actions.

3. **What was tested (from Testing instructions)**  
   - (1) `git rev-parse origin/master origin/development`; `git merge-base --is-ancestor origin/master origin/development`.  
   - (2) Latest **Deploy to amvara9** on **master** via `gh run list`; `gh run view 24773000757 --json conclusion,updatedAt`.  
   - (3) Optional after green — **N/A**.  
   - (4) Manual fallback — **N/A**.
...
4. **Results**
   ...
   | Criterion | Result | Evidence line |
   |---|---|---|
   | 1. Git: refs / lineage | **PASS** | `git merge-base --is-ancestor origin/master origin/development` → exit **0**. SHAs: `origin/master` **7a2c2bd**, `origin/development` **120daf35**. |
   | 2. GitHub Actions: **green** for latest **master** **Deploy to amvara9** | **FAIL** | `gh run list --workflow "Deploy to amvara9" --branch master --limit 8`: latest is **24773000757** (**2026-04-22T10:18:20Z**, **failure**). `gh run view 24773000757 --json conclusion,updatedAt` → **failure**, **updatedAt** **2026-04-22T10:18:30Z** (unchanged vs prior verifications). |
   | 3. Optional after green | **N/A** | Blocked until (2) passes. |
   | 4. Manual fallback | **N/A** | Not run. |
   | Sanity (HTTP) | **INFO** | `curl` → `https://satisfecho.de/api/health` **200**, `https://satisfecho.de/` **200**. |

5. **Overall:** **FAIL** — criterion **2** (green **Deploy to amvara9** for latest **master** deploy in scope). Criterion **1** **PASS**.  
   - **Loop protection (`020-test.md`):** Run **24773000757** GitHub metadata is unchanged (`conclusion`, `updatedAt`); no value in further identical polling until **Actions** secrets (**MARKETING_ARTIFACT_TOKEN** / **GH_TOKEN** per **`config/marketing-sites.json`**) and a **re-run** or new **master** deploy change CI. **Rename:** **TESTING-** → **WIP-**; return to **UNTESTED-** when operators expect a green pipeline.

6. **Product owner feedback**  
   **origin/master** remains an ancestor of **origin/development**, but the latest **master** **Deploy to amvara9** is still failed run **24773000757** from **2026-04-22**. Live HTTP **200** does not satisfy the issue’s requirement for a **successful** deployment workflow on GitHub.
...
8. **Relevant log excerpts (last section)**  
   - `gh run view 24773000757 --repo satisfecho/pos --json conclusion,updatedAt` → `"conclusion":"failure"`, `"updatedAt":"2026-04-22T10:18:30Z"`.  
   - `gh run list --workflow "Deploy to amvara9" --branch master --limit 8` — first entry **databaseId** **24773000757**, **failure**, **2026-04-22T10:18:20Z**.
