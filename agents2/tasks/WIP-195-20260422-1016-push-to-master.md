...
## Test report (2026-04-27T19:31Z–19:36Z UTC — tester \`020-test.md\`, UNTESTED→TESTING→WIP)

1. **Date/time (UTC) and log window**  
   - **Window:** 2026-04-27T19:31Z–2026-04-27T19:36Z.  
   - **Evidence:** Host commands from repo root after \`./scripts/git-sync-development.sh\`: \`git\`, \`gh\`, \`curl\`. Not \`docker logs\` — not required by Testing instructions.

2. **Environment**  
   - **Compose / local app:** N/A.  
   - **Branch:** \`development\` synced (\`git pull --rebase\` already up to date before fetch).  
   - **Remotes (after \`git fetch origin\`):** \`origin/master\` = \`7a2c2bd59b2cfb6cbc6a55ac407993494b17256f\`, \`origin/development\` = \`4335a7b1560545fc1d27fd3baa8c7dd70fe0966f\`.  
   - **GitHub:** \`gh\` CLI to \`satisfecho/pos\` Actions.

3. **What was tested (from Testing instructions)**  
   - (1) \`git rev-parse origin/master origin/development\` and \`git merge-base --is-ancestor origin/master origin/development\`.  
   - (2) Latest **Deploy to amvara9** workflow run on \`master\` (vs **24773000757** / any newer success).  
   - (3) Optional prod URL check after green — **N/A** until (2) is green.  
   - (4) Manual server deploy — **N/A** (not run).
...
4. **Results**
   ...
   | Criterion | Result | Evidence line |
   |---|---|---|
   | 1. Git: refs / lineage | **PASS** | \`origin/master\` SHA above; \`git merge-base --is-ancestor origin/master origin/development\` → exit **0**. |
   | 2. GitHub Actions: **green** latest \`master\` **Deploy to amvara9** | **FAIL** | \`gh run list --limit 15 --repo satisfecho/pos\`: newest **Deploy to amvara9** on \`master\` remains **24773000757** (2026-04-22T10:18:20Z, conclusion **failure**). \`gh run view 24773000757 --json conclusion\` → \`"conclusion":"failure"\`. No newer **success** \`master\` deploy listed before older 2026-04-21 successes. |
   | 3. Optional after green | **N/A** | Blocked until (2) passes. |
   | 4. Manual fallback | **N/A** | Not run. |
   | Sanity (HTTP) | **INFO** | \`curl -s -o /dev/null -w "%{http_code}"\` → \`https://satisfecho.de/api/health\` **200**, \`https://satisfecho.de/\` **200** — does not prove CI deploy green. |

5. **Overall:** **FAIL** — criterion **2** not satisfied (issue \#195: confirm deployment action succeeded on GitHub). Criterion **1** **PASS**.  
   - **Loop protection:** Same failing run **24773000757** and root cause documented across many prior reports in this file; **stop** re-verify cycles until secrets/marketing artifacts are fixed and a **green** \`master\` **Deploy to amvara9** exists or testing scope changes.

6. **Product owner feedback**  
   Branch ancestry is healthy (\`master\` tip is in \`development\` history), but the automated **Deploy to amvara9** run tied to the current \`master\` line has not turned green since **24773000757**. Production HTTP 200 is not a substitute for a successful Actions deploy. Configure **MARKETING_ARTIFACT_TOKEN** / **GH_TOKEN** per repo docs, re-run or redeploy, then hand back **UNTESTED-** for re-test.
...
8. **Relevant log excerpts (last section)**  
   - \`gh run view 24773000757 --json conclusion\` → \`"conclusion":"failure"\`, \`"workflowName":"Deploy to amvara9"\`, \`"url":"https://github.com/satisfecho/pos/actions/runs/24773000757"\`.
