...
## Test report — verification (056 session)

1. **Date/time (UTC)** and log window: **2026-04-22T17:28Z** through **2026-04-22T17:29Z** (`./scripts/git-sync-development.sh` at step start, then **`git fetch`**, **`gh`**, **`curl`** within **~2 minutes**).
...
4. **Results:**
   - **Git remotes:** **PASS** — **`git fetch origin`**, **`git rev-parse origin/master`** → **`7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`** (matches promotion **`7a2c2bd`** prefix), **`git rev-parse origin/development`** → **`3a4fc404e5491ac086a82556c50ed72a6cb5c027`** (**tips differ**; **`development`** ahead). **`git merge-base --is-ancestor origin/master origin/development`** exits **0** (**`master`** contained in **`development`** history).
   - **GitHub Actions Deploy to amvara9:** **FAIL** — **`gh run list --workflow "Deploy to amvara9" --branch master --limit 8`**: latest **`master`** deploy remains **`24773000757`** (**2026-04-22T10:18:20Z**), **`completed`**, **`failure`**. **`gh run view 24773000757 --json`** → **`conclusion":"failure"`**, **`updatedAt":"2026-04-22T10:18:30Z`**. No newer **success** **`master`** deploy after that push.
   - **Optional live check after green deploy:** **N/A** — Deploy workflow for that **`master`** promotion did **not** succeed; **`curl`** **`https://satisfecho.de/api/health`** → **HTTP 200**, **`{"status":"ok"}`** (existing production only; does not validate **`24773000757`** completed SSH/build/smoke).

5. **Overall:** **FAIL** — **Deploy to amvara9** after **`master`** push (**#195**) is **not green**; marketing artifact **PAT** / **`MARKETING_ARTIFACT_TOKEN`** blocker unchanged until repo secrets allow **Fetch marketing site artifacts** and downstream jobs to pass.

6. **Product owner feedback:** Configure **`MARKETING_ARTIFACT_TOKEN`** / **`GH_TOKEN`** in GitHub Actions with **Actions read** on repos in **`config/marketing-sites.json`**, then **Re-run failed jobs** on **`24773000757`** or push **`master`** again after fixes; rename this task to **`UNTESTED-`** when a **green** **`Deploy to amvara9`** run exists for verification.
...
git rev-parse origin/master origin/development
7a2c2bd59b2cfb6cbc6a55ac407993494b17256f
3a4fc404e5491ac086a82556c50ed72a6cb5c027

gh run view 24773000757 --json conclusion,status,updatedAt,name
{"conclusion":"failure","name":"Deploy to amvara9","status":"completed","updatedAt":"2026-04-22T10:18:30Z"}

curl -sS https://satisfecho.de/api/health
{"status":"ok"}
```

9. **Loop protection:** Sessions **055** and earlier already documented **>3** cycles on the same failing run **`24773000757`**; outcome unchanged until CI produces a superseding **success** **`master`** deploy.
