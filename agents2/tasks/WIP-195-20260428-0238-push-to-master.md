...
## Test report (2026-04-28T04:55Z–2026-04-28T04:58Z UTC, 020-tester)

1. **Date/time (UTC) and log window**  
   - **Window:** 2026-04-28T04:55Z–2026-04-28T04:58Z (approx.).  
   - **Commands:** `git` / `gh` / `curl` on host at `/Users/raro42/projects/pos2` after `./scripts/git-sync-development.sh` and `git fetch origin` (not `docker logs` — not required by Testing instructions).

2. **Environment**  
   - **Compose / local app:** N/A (not in Testing instructions).  
   - **Branch:** `development` synced before task file edits (workspace sync).  
   - **Remotes (after `git fetch origin`):** `origin/master` = `7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`, `origin/development` = `e5c3656cc0047fc5e4db279524120938230f20a4`.  
   - **Evidence:** `gh` against `satisfecho/pos` Actions.

3. **What was tested (from “Testing instructions”)**  
   - (1) `git rev-parse origin/master origin/development` and ancestor check.  
   - (2) **Deploy to amvara9** — latest **`master`** workflow run (**24773000757** or newer **success**).  
   - (3) Optional live check after **green** — N/A (no green **master** deploy).  
   - (4) Manual server deploy — N/A (not run).
...
4. **Results**
   ...
   |---|---|---|
   | 1. Git: `origin/master` / `origin/development` and lineage | **PASS** | SHAs above; `git merge-base --is-ancestor origin/master origin/development` → exit **0**. |
   | 2. GitHub Actions: **green** for latest **`master`** **Deploy to amvara9** | **FAIL** | `gh run list --repo satisfecho/pos --workflow "Deploy to amvara9" --branch master --limit 12`: most recent is **24773000757** (2026-04-22T10:18:20Z) **failure**. `gh run view 24773000757 --json conclusion` → **`"failure"`**. No newer **success** on **`master`** after this run in the listed window. |
   | 3. Optional after green | **N/A** | Not applicable until (2) passes. |
   | 4. Manual fallback | **N/A** | Not run. |
   | Sanity (HTTP) | **INFO** | `curl` → `https://satisfecho.de/api/health` **200**, `https://satisfecho.de/` **200**; does not prove CI deploy success. |

5. **Overall: FAIL**  
   - Criterion **2** fails: issue requirement to confirm **Deploy to amvara9** **succeeded** on GitHub is **not** met. **Loop protection:** Repeated verification cycles against the **same** failing run **24773000757** without a secrets/workflow fix yield the same outcome; stop expecting a different result until **`MARKETING_ARTIFACT_TOKEN` / `GH_TOKEN`** (or equivalent) is configured per **`config/marketing-sites.json`** and a **green** workflow completes.

6. **Product owner feedback**  
   Remote git state looks healthy (**master** contained in **development** history), but automated **`master`** deploy is still **red** on the latest relevant Actions run. Production HTTP 200 responses are not a substitute for a successful deploy pipeline; configure PAT/secrets and re-run deploy, then return the task as **UNTESTED-** for another pass.
...
8. **Relevant log excerpts (last section)**  
   - `gh run view 24773000757 --json conclusion,updatedAt` → **`failure`**, **updatedAt** `2026-04-22T10:18:30Z`.  
   - Step-level logs unchanged from prior entries in this file (marketing token / placeholder bundles); no new successful **`master`** deploy superseded this run during this verification window.
