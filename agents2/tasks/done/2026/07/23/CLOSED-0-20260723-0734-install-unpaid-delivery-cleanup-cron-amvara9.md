...
- `the production host`: crontab has `reset-demo-data-on-server.sh` only; **no** `cleanup-unpaid-public-delivery-on-server.sh`; script path missing under `the deploy directory/scripts/` (not yet on server checkout / `origin/master`)
...
- Ensure the wrapper + seed module are present on the amvara9 checkout (`the deploy directory`) — promote **`development` → `master`** and deploy (or equivalent pull) so `scripts/cleanup-unpaid-public-delivery-on-server.sh` exists and is executable; do not invent a second cleanup path
...
1. **Script on amvara9:**  
   `the production host 'test -x the deploy directory/scripts/cleanup-unpaid-public-delivery-on-server.sh && ls -la the deploy directory/scripts/cleanup-unpaid-public-delivery-on-server.sh'`  
   Expect executable present; server HEAD should include it (`git -C the deploy directory rev-parse --short HEAD` near `6df7867a` or later master).

2. **Dry-run exits 0:**  
   `the production host 'cd the deploy directory && ./scripts/cleanup-unpaid-public-delivery-on-server.sh --dry-run'`  
   Expect exit 0 and a `[dry-run] matched=… cancelled=0 …` line.

3. **Cleanup cron installed:**  
   `the production host 'crontab -l | grep cleanup-unpaid-public-delivery-on-server.sh'`  
   Expect: `15 * * * * cd the deploy directory && ./scripts/cleanup-unpaid-public-delivery-on-server.sh >>/var/log/pos-unpaid-public-delivery-cleanup.log 2>&1`

4. **Demo-reset cron still present (unchanged):**  
   `the production host 'crontab -l | grep reset-demo-data-on-server.sh'`  
   Expect: `0 4 * * * cd the deploy directory && ./scripts/reset-demo-data-on-server.sh >>/var/log/pos-demo-reset.log 2>&1`  
...
2. **Environment:** Production amvara9 (`the deploy directory`), server HEAD `6df7867a` (`origin/master`). Local branch `development` @ `9a95a899` (tester only; verification on amvara9). No `BASE_URL` browser flow.
...
4. **Results:**
   - Script on amvara9: **PASS** — `-rwxr-xr-x` `the deploy directory/scripts/cleanup-unpaid-public-delivery-on-server.sh`; `git rev-parse --short HEAD` → `6df7867a`.
   - Dry-run exits 0: **PASS** — `[dry-run] matched=0 cancelled=0 ttl_hours=2 cutoff=2026-07-23T05:41:44.124528+00:00`; `DRY_RUN_EXIT=0`.
   - Cleanup cron installed: **PASS** — `15 * * * * cd the deploy directory && ./scripts/cleanup-unpaid-public-delivery-on-server.sh >>/var/log/pos-unpaid-public-delivery-cleanup.log 2>&1` (exactly one crontab match).
   - Demo-reset cron still present: **PASS** — `0 4 * * * cd the deploy directory && ./scripts/reset-demo-data-on-server.sh >>/var/log/pos-demo-reset.log 2>&1`; cleanup not merged into that line.
...
```
-rwxr-xr-x 1 root root 1214 Jul 23 07:37 the deploy directory/scripts/cleanup-unpaid-public-delivery-on-server.sh
6df7867a
[dry-run] matched=0 cancelled=0 ttl_hours=2 cutoff=2026-07-23T05:41:44.124528+00:00
DRY_RUN_EXIT=0
15 * * * * cd the deploy directory && ./scripts/cleanup-unpaid-public-delivery-on-server.sh >>/var/log/pos-unpaid-public-delivery-cleanup.log 2>&1
0 4 * * * cd the deploy directory && ./scripts/reset-demo-data-on-server.sh >>/var/log/pos-demo-reset.log 2>&1
```
