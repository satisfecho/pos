...
- Merge/promote only production-ready changes; confirm **#294** SSH port fix is included so GHA deploy uses **`-p the configured SSH port`** (not port 22).
- Watch **Deploy to amvara9** for the pushed **`master`** commit; document run URL and conclusion. If GHA still fails, fall back per **`docs/0001-ci-cd-amvara9.md`**: `the production host 'cd the deploy directory && git fetch origin && git checkout -f master && git reset --hard origin/master && bash scripts/deploy-amvara9.sh'`.
...
- **Deploy to amvara9** run **29255611539** — **success** (2m19s); all steps green including SSH checkout on port **the configured SSH port**, marketing rsync, build/restart, and workflow smoke test.
...
| GHA Deploy to amvara9 | **PASS** | Run https://github.com/satisfecho/pos/actions/runs/29255611539 — **success** (2m19s). All steps green: Set up SSH (`DEPLOY_SSH_PORT: the configured SSH port`), Checkout latest code on amvara9, Sync marketing sites, Build and restart stack, Smoke test. |
...
**PASS** — all criteria met. Production is on release **2.1.15** @ **`59bd1dec`**; deploy pipeline uses SSH port **the configured SSH port** and completed successfully.
...
```
Set up SSH: success (DEPLOY_SSH_PORT: the configured SSH port)
