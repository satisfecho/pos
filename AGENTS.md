...
**Long-running repeat (optional):** `scripts/go-ahead-loop.sh` pulls git (`--rebase --autostash`) and reruns Docker pytest plus `npm run test:landing-version` on an interval for hours (default ~8h). Requires `GO_AHEAD_LOOP=1`; see `docs/testing.md`.
